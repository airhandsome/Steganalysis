...
def hadoop_53(local_img_path,local_txt_path,local_mod_path,local_output,taskid):
    ...
    copy_mod_cmd= "fs -copyFromLocal " + local_mod_path + " " + fs_task_path+"temp.model"
    run_hadoopcmd(copy_txt_cmd)
    run_hadoopcmd(copy_img_cmd)
    run_hadoopcmd(copy_mod_cmd)

    fs_task_path=hdfs_root+str(taskid)+"/"
    file_path = "/home/test_data/5_mapper_reducer/53_train_predict/mapper_train_predict.py,/home/test_data/5_mapper_reducer/53_train_predict/reducer_train_predict.py" 
...
def main():
    ...
    local_mod_path=data_root+"task/"+str(taskid)+"/3.model"

    local_output=data_root+"task/"+str(taskid)+"/result"

    get_img_path(local_img_path, local_txt_path,taskid)

    hadoop_53(local_img_path,local_txt_path,local_mod_path,local_output,taskid)
